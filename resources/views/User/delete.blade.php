<x-app-layout>
    <x-slot name="header">
        <x-policy-ui-shared:default-header header="{{ __('Delete User') }} '{{ $item->name }}'" />
    </x-slot>

    <x-policy-ui-shared:outer-modal-layout>
        <form method="POST" action="{{ route('policy-ui.user.destroy', ['user' => $item->id]) }}">
            @method('DELETE')
            @csrf
            ...
            <div class="flex flex-row">
                ...
                <x-policy-ui-shared:inner-modal-layout>
                    <x-policy-ui-shared:default-modal-title title="{{ _('Delete user') }}" />
                    <x-policy-ui-shared:default-modal-content>
                        <p class="text-sm text-gray-500">
                            Are you sure you want to delete this user '{{ $item->name }}' ? This

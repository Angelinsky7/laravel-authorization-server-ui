...
<div x-data="window.policy.alpineJs.datePicker({
        ...
        timeVisible: {!! json_encode($time_visible) !!}
    })">
    <div class="container mx-auto">
        <div class="w-64">
            <div class="relative">
                ...
                <x-policy-ui-shared:input-mask class="w-full pl-4 pr-10 py-3 leading-none rounded-lg shadow-sm focus:outline-none text-gray-600 font-medium focus:ring focus:ring-blue-600 focus:ring-opacity-50"
                    ...
                    <div x-ref="panel">
                        <div class="bg-white rounded-lg shadow p-4 absolute"
                            ...
                            <div class="flex justify-between items-center mb-2">
                                <div>
                                    <span x-text="_getMonthName(month)" class="text-lg font-bold text-gray-800"></span>
                            ...
                            <div class="flex flex-wrap mb-3 -mx-1">
                                <template x-for="(day, index) in _getDays()" :key="index">
                                ...
                                <template x-for="(date, dateIndex) in no_of_days" :key="dateIndex">
                                    <div class="px-1 mb-1 w-[14.28%] h-[14.28%]">
                                        <div class="cursor-pointer text-center text-sm leading-loose rounded-full transition ease-in-out duration-100"
                                             x-on:click="_setDateValue(date)"
                                             x-text="date"
                                             x-bind:class="{
                                                'bg-indigo-200': _isToday(date) == true,
                                                'text-gray-600 hover:bg-indigo-200': _isToday(date) == false && _isSelectedDay(date) == false,
                                                'bg-indigo-500 text-white hover:bg-opacity-75': _isSelectedDay(date) == true
                                              }">
                                        </div>
                                    </div>
                                </template>
                            ...
                            @if ($time_visible)
                                <x-policy-ui-shared:time-picker data-js-timePickerControl

<x-app-layout>
    <x-slot name="header">
        <x-policy-ui-shared:default-header header="{{ __('Delete Scope') }} '{{ $item->name }}'" />
    </x-slot>

    <x-policy-ui-shared:outer-modal-layout>
        <form method="POST" action="{{ route('policy-ui.scope.destroy', ['scope' => $item->id]) }}">
            @method('DELETE')
            @csrf
            ...
            <div class="flex flex-row">
                ...
                <x-policy-ui-shared:inner-modal-layout>
                    <x-policy-ui-shared:default-modal-title title="{{ _('Delete scope') }}" />
